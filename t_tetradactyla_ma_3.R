...
tt_ma_original <- read_excel("D:/CETESB/Rodovias e pontos de ausencia/bin_tt_ma_1999_2023.xlsx", sheet = 1,na = c(" ", "NA")) |>
  st_as_sf(coords = c("coord_x", "coord_y"), crs=4674)|>
  st_transform(5880)|>
  select(-ID)

write_sf(tt_ma_original, "tt_ma.shp")


tt_ma <- st_read("tt_ma.shp") |>
  st_transform(5880)
...
tt_ma <- tt_ma |>
  select(-Referencia, -Ocorrencia, -Data, -Mes)
#### Igualando presenca e ausencia ####

tt_ma |> 
  count(Bin)

# Identifica as linhas com Bin == 0
zeros <- which(tt_ma$Bin == 0)

# Seleciona zeros aleatórios para remover
remove_rows <- sample(zeros, min(516, length(zeros)))

# Filtra o tibble
tt_ma_total <- tt_ma[-remove_rows, ]

View(tt_ma_total)

write_xlsx(tt_ma_total, "tt_ma_total.xlsx")

write_sf(tt_ma_total, "tt_ma_total.shp")
...
bf_tt_ma_2023_500m <-tt_ma_total |>
  filter(Ano == "2023") |>
  st_buffer(dist = 500)

bf_tt_ma_2022_500m <-tt_ma_total |>
  filter(Ano == "2022") |>
  st_buffer(dist = 500)

bf_tt_ma_2021_500m <-tt_ma_total |>
  filter(Ano == "2021") |>
  st_buffer(dist = 500)

bf_tt_ma_2020_500m <-tt_ma_total |>
  filter(Ano == "2020") |>
  st_buffer(dist = 500)

bf_tt_ma_2019_500m <-tt_ma_total |>
  filter(Ano == "2019") |>
  st_buffer(dist = 500)

bf_tt_ma_2018_500m <-tt_ma_total |>
  filter(Ano == "2018") |>
  st_buffer(dist = 500)

bf_tt_ma_2017_500m <-tt_ma_total |>
  filter(Ano == "2017") |>
  st_buffer(dist = 500)

bf_tt_ma_2016_500m <-tt_ma_total |>
  filter(Ano == "2016") |>
  st_buffer(dist = 500)

bf_tt_ma_2015_500m <-tt_ma_total |>
  filter(Ano == "2015") |>
  st_buffer(dist = 500)

bf_tt_ma_2014_500m <-tt_ma_total |>
  filter(Ano == "2014") |>
  st_buffer(dist = 500)

bf_tt_ma_2013_500m <-tt_ma_total |>
  filter(Ano == "2013") |>
  st_buffer(dist = 500)

bf_tt_ma_2012_500m <-tt_ma_total |>
  filter(Ano == "2012") |>
  st_buffer(dist = 500)

bf_tt_ma_2011_500m <-tt_ma_total |>
  filter(Ano == "2011") |>
  st_buffer(dist = 500)

bf_tt_ma_2010_500m <-tt_ma_total |>
  filter(Ano == "2010") |>
  st_buffer(dist = 500)

bf_tt_ma_2009_500m <-tt_ma_total |>
  filter(Ano == "2009") |>
  st_buffer(dist = 500)

bf_tt_ma_2008_500m <-tt_ma_total |>
  filter(Ano == "2008") |>
  st_buffer(dist = 500)

bf_tt_ma_2007_500m <-tt_ma_total |>
  filter(Ano == "2007") |>
  st_buffer(dist = 500)

bf_tt_ma_2006_500m <-tt_ma_total |>
  filter(Ano == "2006") |>
  st_buffer(dist = 500)

bf_tt_ma_2005_500m <-tt_ma_total |>
  filter(Ano == "2005") |>
  st_buffer(dist = 500)

bf_tt_ma_2004_500m <-tt_ma_total |>
  filter(Ano == "2004") |>
  st_buffer(dist = 500)

bf_tt_ma_2003_500m <-tt_ma_total |>
  filter(Ano == "2003") |>
  st_buffer(dist = 500)

bf_tt_ma_2002_500m <-tt_ma_total |>
  filter(Ano == "2002") |>
  st_buffer(dist = 500)

bf_tt_ma_2001_500m <-tt_ma_total |>
  filter(Ano == "2001") |>
  st_buffer(dist = 500)

bf_tt_ma_2000_500m <-tt_ma_total |>
  filter(Ano == "2000") |>
  st_buffer(dist = 500)

bf_tt_ma_1999_500m <-tt_ma_total |>
  filter(Ano == "1999") |>
  st_buffer(dist = 500)
...
write_xlsx(lsm_tt_ma_500m, "lsm_tt_ma_500m.xlsx")

#lsm_tt_ma_500m <- read_excel("D:/Analise estatistica mestrado/lsm_tt_ma_500m.xlsx", sheet = 1,na = c(" ", "NA")) 

#### Gerando buffer 1000m ####


bf_tt_ma_2023_1000m <-tt_ma_total |>
  filter(Ano == "2023") |>
  st_buffer(dist = 1000)

bf_tt_ma_2022_1000m <-tt_ma_total |>
  filter(Ano == "2022") |>
  st_buffer(dist = 1000)

bf_tt_ma_2021_1000m <-tt_ma_total |>
  filter(Ano == "2021") |>
  st_buffer(dist = 1000)

bf_tt_ma_2020_1000m <-tt_ma_total |>
  filter(Ano == "2020") |>
  st_buffer(dist = 1000)

bf_tt_ma_2019_1000m <-tt_ma_total |>
  filter(Ano == "2019") |>
  st_buffer(dist = 1000)

bf_tt_ma_2018_1000m <-tt_ma_total |>
  filter(Ano == "2018") |>
  st_buffer(dist = 1000)

bf_tt_ma_2017_1000m <-tt_ma_total |>
  filter(Ano == "2017") |>
  st_buffer(dist = 1000)

bf_tt_ma_2016_1000m <-tt_ma_total |>
  filter(Ano == "2016") |>
  st_buffer(dist = 1000)

bf_tt_ma_2015_1000m <-tt_ma_total |>
  filter(Ano == "2015") |>
  st_buffer(dist = 1000)

bf_tt_ma_2014_1000m <-tt_ma_total |>
  filter(Ano == "2014") |>
  st_buffer(dist = 1000)

bf_tt_ma_2013_1000m <-tt_ma_total |>
  filter(Ano == "2013") |>
  st_buffer(dist = 1000)

bf_tt_ma_2012_1000m <-tt_ma_total |>
  filter(Ano == "2012") |>
  st_buffer(dist = 1000)

bf_tt_ma_2011_1000m <-tt_ma_total |>
  filter(Ano == "2011") |>
  st_buffer(dist = 1000)

bf_tt_ma_2010_1000m <-tt_ma_total |>
  filter(Ano == "2010") |>
  st_buffer(dist = 1000)

bf_tt_ma_2009_1000m <-tt_ma_total |>
  filter(Ano == "2009") |>
  st_buffer(dist = 1000)

bf_tt_ma_2008_1000m <-tt_ma_total |>
  filter(Ano == "2008") |>
  st_buffer(dist = 1000)

bf_tt_ma_2007_1000m <-tt_ma_total |>
  filter(Ano == "2007") |>
  st_buffer(dist = 1000)

bf_tt_ma_2006_1000m <-tt_ma_total |>
  filter(Ano == "2006") |>
  st_buffer(dist = 1000)

bf_tt_ma_2005_1000m <-tt_ma_total |>
  filter(Ano == "2005") |>
  st_buffer(dist = 1000)

bf_tt_ma_2004_1000m <-tt_ma_total |>
  filter(Ano == "2004") |>
  st_buffer(dist = 1000)

bf_tt_ma_2003_1000m <-tt_ma_total |>
  filter(Ano == "2003") |>
  st_buffer(dist = 1000)

bf_tt_ma_2002_1000m <-tt_ma_total |>
  filter(Ano == "2002") |>
  st_buffer(dist = 1000)

bf_tt_ma_2001_1000m <-tt_ma_total |>
  filter(Ano == "2001") |>
  st_buffer(dist = 1000)

bf_tt_ma_2000_1000m <-tt_ma_total |>
  filter(Ano == "2000") |>
  st_buffer(dist = 1000)

bf_tt_ma_1999_1000m <-tt_ma_total |>
  filter(Ano == "1999") |>
  st_buffer(dist = 1000)
...
bf_tt_ma_2023_2000m <-tt_ma_total |>
  filter(Ano == "2023") |>
  st_buffer(dist = 2000)

bf_tt_ma_2022_2000m <-tt_ma_total |>
  filter(Ano == "2022") |>
  st_buffer(dist = 2000)

bf_tt_ma_2021_2000m <-tt_ma_total |>
  filter(Ano == "2021") |>
  st_buffer(dist = 2000)

bf_tt_ma_2020_2000m <-tt_ma_total |>
  filter(Ano == "2020") |>
  st_buffer(dist = 2000)

bf_tt_ma_2019_2000m <-tt_ma_total |>
  filter(Ano == "2019") |>
  st_buffer(dist = 2000)

bf_tt_ma_2018_2000m <-tt_ma_total |>
  filter(Ano == "2018") |>
  st_buffer(dist = 2000)

bf_tt_ma_2017_2000m <-tt_ma_total |>
  filter(Ano == "2017") |>
  st_buffer(dist = 2000)

bf_tt_ma_2016_2000m <-tt_ma_total |>
  filter(Ano == "2016") |>
  st_buffer(dist = 2000)

bf_tt_ma_2015_2000m <-tt_ma_total |>
  filter(Ano == "2015") |>
  st_buffer(dist = 2000)

bf_tt_ma_2014_2000m <-tt_ma_total |>
  filter(Ano == "2014") |>
  st_buffer(dist = 2000)

bf_tt_ma_2013_2000m <-tt_ma_total |>
  filter(Ano == "2013") |>
  st_buffer(dist = 2000)

bf_tt_ma_2012_2000m <-tt_ma_total |>
  filter(Ano == "2012") |>
  st_buffer(dist = 2000)

bf_tt_ma_2011_2000m <-tt_ma_total |>
  filter(Ano == "2011") |>
  st_buffer(dist = 2000)

bf_tt_ma_2010_2000m <-tt_ma_total |>
  filter(Ano == "2010") |>
  st_buffer(dist = 2000)

bf_tt_ma_2009_2000m <-tt_ma_total |>
  filter(Ano == "2009") |>
  st_buffer(dist = 2000)

bf_tt_ma_2008_2000m <-tt_ma_total |>
  filter(Ano == "2008") |>
  st_buffer(dist = 2000)

bf_tt_ma_2007_2000m <-tt_ma_total |>
  filter(Ano == "2007") |>
  st_buffer(dist = 2000)

bf_tt_ma_2006_2000m <-tt_ma_total |>
  filter(Ano == "2006") |>
  st_buffer(dist = 2000)

bf_tt_ma_2005_2000m <-tt_ma_total |>
  filter(Ano == "2005") |>
  st_buffer(dist = 2000)

bf_tt_ma_2004_2000m <-tt_ma_total |>
  filter(Ano == "2004") |>
  st_buffer(dist = 2000)

bf_tt_ma_2003_2000m <-tt_ma_total |>
  filter(Ano == "2003") |>
  st_buffer(dist = 2000)

bf_tt_ma_2002_2000m <-tt_ma_total |>
  filter(Ano == "2002") |>
  st_buffer(dist = 2000)

bf_tt_ma_2001_2000m <-tt_ma_total |>
  filter(Ano == "2001") |>
  st_buffer(dist = 2000)

bf_tt_ma_2000_2000m <-tt_ma_total |>
  filter(Ano == "2000") |>
  st_buffer(dist = 2000)

bf_tt_ma_1999_2000m <-tt_ma_total |>
  filter(Ano == "1999") |>
  st_buffer(dist = 2000)
...
write_xlsx(lsm_tt_ma_2000m, "lsm_tt_ma_2000m.xlsx")

#lsm_tt_ma_2000m <- read_excel("D:/Analise estatistica mestrado/lsm_tt_ma_2000m.xlsx", sheet = 1,na = c(" ", "NA")) 

#### Distancia massa dagua ####

dist_agua_tt_ma <- st_distance(tt_ma, agua_ma)

tt_ma$dist_min_agua <- as.numeric(apply(dist_agua_tt_ma, 1, min))

dist_min_agua_ma_tt <- st_drop_geometry(tt_ma[, c("id_unico", "dist_min_agua")])
...
distancias_result <- calcular_dist_minima(tt_ma, UC)
...
den_500m <- data.frame(id_unico = lsm_tt_ma_500m$id_unico_500m) |>
  left_join(den_rod_500m, by = "id_unico") |>
  mutate(den_rod_500m = ifelse(is.na(den_rod_500m), 0, as.numeric(den_rod_500m)))

lsm_tt_ma_500m$den_rod_500m <- den_500m$den_rod_500m

den_rod_2023 <- st_drop_geometry(lsm_tt_ma_500m[, c("id_unico_500m", "den_rod_500m")])
...
lsm_tt_ma_500m_uni <- lsm_tt_ma_500m |>
...
lsm_tt_ma_total <- lsm_tt_ma_500m_uni %>%
  ...
  left_join(dist_min_agua_ma_tt, by = 'id_unico') %>%
  left_join(dist_min_uc_tt_ma, by = 'id_unico')


colum_na <- lsm_tt_ma_total|>
  summarise(across(everything(), ~sum(is.na(.))))

glimpse(colum_na)

lsm_tt_ma_total<- lsm_tt_ma_total %>% 
  mutate(across(where(is.numeric), ~coalesce(., 0)))

write_xlsx(lsm_tt_ma_total, "lsm_tt_ma_total.xlsx")

lsm_tt_ma_total <- read_excel("lsm_tt_ma_total.xlsx", sheet = 1,na = c(" ", "NA"))
...
id_cols <- lsm_tt_ma_total |> select(id_unico, Ano, Bin, median_shape_500m, median_shape_1000m, median_shape_2000m)

num_cols <- lsm_tt_ma_total |> select(-id_unico, -Ano, -Bin, -median_shape_500m, -median_shape_1000m, -median_shape_2000m)
...
lsm_tt_ma_std <- bind_cols(id_cols, stan)

# Verificar o resultado
glimpse(lsm_tt_ma_std)

write_xlsx(lsm_tt_ma_std, "lsm_tt_ma_std.xlsx")

lsm_tt_ma_std$Bin <- as.factor(lsm_tt_ma_std$Bin)


var_tt_ma <- lsm_tt_ma_std 

### Unindo todos os dados ####

# Obtendo a rodovia
rod_tt_ma<-read_sf("D:/CETESB/Rodovias e pontos de ausencia/rod_tt_ma.shp")

glimpse(rod_tt_ma)

rod_tt_ma <- st_transform(rod_tt_ma, st_crs(tt_ma))

indices_mais_proximos <- st_nearest_feature(tt_ma, rod_tt_ma)

tt_ma$Rodovia <- rod_tt_ma$sigla_2[indices_mais_proximos]

tt_ma_rod <- tt_ma|>
  select(id_unico, Rodovia)

# Renomeando referencias

tt_ma_original_renomeado <- tt_ma_original %>%
  mutate(Referencia = case_when(
    # Se começar com 'CETESB' (independente do que vem depois), vira apenas 'CETESB'
    str_starts(Referencia, "CETESB") ~ "CETESB",
    
    # Se for NA, vira 'Outro'
    is.na(Referencia) ~ "Outro",
    
    # Mantém o valor original caso não se encaixe nas regras acima
    TRUE ~ Referencia
  ))

# Unies

tt_ma_coords <- st_join(tt_ma_rod, tt_ma_original_renomeado, join = st_equals)

var_tt_ma_total <- right_join(tt_ma_coords, var_tt_ma, by = "id_unico") |>
  select(c(-Ano.y, -Bin.y))|>
  rename(Bin = Bin.x,
         Ano = Ano.x)

var_tt_ma_total <- var_tt_ma_total %>%
  mutate(X = st_coordinates(.)[,1],
         Y = st_coordinates(.)[,2])

var_tt_ma_total$id_unico <- as.factor(var_tt_ma_total$id_unico)
var_tt_ma_total$Referencia <- as.factor(var_tt_ma_total$Referencia)
var_tt_ma_total$Rodovia <- as.factor(var_tt_ma_total$Rodovia)

glimpse(var_tt_ma_total)

#### Modelos ####

num_var_tt_ma <- var_tt_ma_total|>
  dplyr::select(-id_unico, - Bin, -Ano, -median_shape_500m, -median_shape_1000m, -median_shape_2000m, -Referencia, -Ocorrencia, -Rodovia, -Data, -X, -Y, -pland_12_500m, -pland_12_1000m, -pland_12_2000m)|>
  st_drop_geometry()


# 1. Obter os nomes das colunas
vars <- colnames(num_var_tt_ma)

# 2. Identificar variáveis de distância (sem escala) e variáveis de escala
dist_vars <- vars[!str_detect(vars, "_\\d+m$")]
escala_vars <- vars[str_detect(vars, "_\\d+m$")]

# 3. Criar uma tabela com a escala de cada variável de escala
meta_escala <- tibble(coluna = escala_vars) %>%
  mutate(escala = str_extract(coluna, "\\d+m$"))

# 4. Criar um conjunto de dados expandido:
# Para cada grupo de escala, adicionamos as variáveis de distância
combinacoes_tt_ma <- meta_escala %>%
  group_by(escala) %>%
  summarise(
    # Combinamos as variáveis daquela escala com TODAS as variáveis de distância
    # E também combinamos as variáveis da escala entre si
    lista_vars = list(c(coluna, dist_vars))
  ) %>%
  # Gerar combinações de 2 a 2 para cada grupo (agora contendo escalas + distâncias)
  mutate(
    comb = purrr::map(lista_vars, ~as.data.frame(t(combn(.x, 2))))
  ) %>%
  unnest(comb) %>%
  rename(var1 = V1, var2 = V2) %>%
  select(var1, var2) %>%
  # Adicionar combinações entre as próprias variáveis de distância (apenas uma vez)
  bind_rows(
    as.data.frame(t(combn(dist_vars, 2))) %>% 
      rename(var1 = V1, var2 = V2)
  )

# Resultado final
glimpse(combinacoes_tt_ma)
...
# Extrair combinações
comb <- var_glm(combinacoes_tt_ma)


# 1. Redefina a função com o parâmetro explícito
criar_formulas_glm <- function(var_individuais, pares, resposta = "Bin") {
...
# 2. REGENERE as fórmulas (agora com o efeito aleatório garantido)
formulas_glm <- criar_formulas_glm(comb$modelos_individuais, comb$modelos_pares)

# 3. VERIFIQUE se o ano apareceu agora
print(formulas_glm$ind_ed_3_1000m) 
# Deve imprimir: Bin ~ ed_3_1000m + (1 | Ano)


# 4. Rode o loop usando a lista correta
modelos <- list()
for (nome in names(formulas_glm)) {
  modelos[[nome]] <- glm(formulas_glm[[nome]], 
                         data = var_tt_ma_total, 
                         family = binomial)
}


preparar_mumin <- function(lista_modelos, max_vif = 5) {
...
modelos_vif<-preparar_mumin(modelos) # 355 → 324 modelos (VIF ≤ 5 )
...
simulationOutput <- DHARMa::simulateResiduals(fittedModel = melhor_modelo_1, plot = TRUE) # OK

probs_1 <- predict(melhor_modelo_1, type = "response")
roc_obj_1 <- roc(var_tt_ma_total$Bin, probs_1)
auc(roc_obj_1) # 0.6242

rcompanion::nagelkerke(melhor_modelo_1) # nagelkerke: 0.0589781 p-value: 7.2696e-11

# Melhor modelo 2

melhor_modelo_2 <- MuMIn::get.models(aic_tabela, subset = 2)[[1]]
summary(melhor_modelo_2)
simulationOutput <- DHARMa::simulateResiduals(fittedModel = melhor_modelo_2, plot = TRUE) # OK

probs_2 <- predict(melhor_modelo_2, type = "response")
roc_obj_2 <- roc(var_tt_ma_total$Bin, probs_2)
auc(roc_obj_2) # 0.6225

rcompanion::nagelkerke(melhor_modelo_2) # nagelkerke: 0.0591097 p-value: 3.8407e-10

#### Exportando resultados ####

# 2. Criar a lista de modelos (supondo que seus modelos estejam em uma lista)
lista_modelos <- get.models(aic_tabela, subset = TRUE)

# 3. Extrair métricas dos modelos (AIC, Delta, AICw)
tabela_selecao <- as.data.frame(aic_tabela)

tabela_selecao$modelo <- rownames(tabela_selecao)

# 4. Criar a tabela detalhada com coeficientes e p-valores
tabela_final <- map_df(lista_modelos, function(m) {
  tidy(m) %>% 
    mutate(AIC = AIC(m)) # Adiciona o AIC de cada modelo
}, .id = "modelo") %>%
  left_join(tabela_selecao %>% select(modelo, delta, weight), by = "modelo")

# 5. Adicionar Fator de Evidência (Evidence Ratio)
# Geralmente calculado como w_i / w_min ou w_best / w_i
tabela_final <- tabela_final %>%
  mutate(evid_ratio = max(weight) / weight) %>%
  filter(delta <= 2)

# 6. Exportar para Excel ou CSV
write_xlsx(tabela_final, "tabela_modelos_2.xlsx")
