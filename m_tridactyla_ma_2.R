...
mt_ma <- mt_ma |>
  select(-Processo, -Ocorrencia, -Data, -Mes)
...
zeros <- which(mt_ma$Bin == 0)

# Seleciona zeros aleatórios para remover
remove_rows <- sample(zeros, min(739, length(zeros)))

# Filtra o tibble
mt_ma <- mt_ma[-remove_rows, ]

View(mt_ma)

write_xlsx(mt_ma, "mt_ma.xlsx")
...
write_xlsx(lsm_mt_ma_500m, "lsm_mt_ma_500m.xlsx")


#### Gerando buffer 3500m ####


bf_mt_ma_2023_3500m <-mt_ma |>
  filter(Ano == "2023") |>
  st_buffer(dist = 3500)

bf_mt_ma_2022_3500m <-mt_ma |>
  filter(Ano == "2022") |>
  st_buffer(dist = 3500)

bf_mt_ma_2021_3500m <-mt_ma |>
  filter(Ano == "2021") |>
  st_buffer(dist = 3500)

bf_mt_ma_2020_3500m <-mt_ma |>
  filter(Ano == "2020") |>
  st_buffer(dist = 3500)

bf_mt_ma_2019_3500m <-mt_ma |>
  filter(Ano == "2019") |>
  st_buffer(dist = 3500)

bf_mt_ma_2018_3500m <-mt_ma |>
  filter(Ano == "2018") |>
  st_buffer(dist = 3500)

bf_mt_ma_2017_3500m <-mt_ma |>
  filter(Ano == "2017") |>
  st_buffer(dist = 3500)

bf_mt_ma_2016_3500m <-mt_ma |>
  filter(Ano == "2016") |>
  st_buffer(dist = 3500)

bf_mt_ma_2015_3500m <-mt_ma |>
  filter(Ano == "2015") |>
  st_buffer(dist = 3500)

bf_mt_ma_2014_3500m <-mt_ma |>
  filter(Ano == "2014") |>
  st_buffer(dist = 3500)

bf_mt_ma_2013_3500m <-mt_ma |>
  filter(Ano == "2013") |>
  st_buffer(dist = 3500)

bf_mt_ma_2012_3500m <-mt_ma |>
  filter(Ano == "2012") |>
  st_buffer(dist = 3500)

bf_mt_ma_2011_3500m <-mt_ma |>
  filter(Ano == "2011") |>
  st_buffer(dist = 3500)

bf_mt_ma_2010_3500m <-mt_ma |>
  filter(Ano == "2010") |>
  st_buffer(dist = 3500)
...
output_dir <- "ras_mt_ma_2016_3500m"
ras_mt_ma_2016_3500m <- list.files(output_dir, pattern = "\\.tif$", full.names = TRUE) %>%
  set_names(~ str_remove(basename(.), "ras_mt_ma_2016_3500m\\.tif$")) %>%
  map(rast) 
...
bf_mt_ma_2023_2000m <-mt_ma |>
  filter(Ano == "2023") |>
  st_buffer(dist = 2000)

bf_mt_ma_2022_2000m <-mt_ma |>
  filter(Ano == "2022") |>
  st_buffer(dist = 2000)

bf_mt_ma_2021_2000m <-mt_ma |>
  filter(Ano == "2021") |>
  st_buffer(dist = 2000)

bf_mt_ma_2020_2000m <-mt_ma |>
  filter(Ano == "2020") |>
  st_buffer(dist = 2000)

bf_mt_ma_2019_2000m <-mt_ma |>
  filter(Ano == "2019") |>
  st_buffer(dist = 2000)

bf_mt_ma_2018_2000m <-mt_ma |>
  filter(Ano == "2018") |>
  st_buffer(dist = 2000)

bf_mt_ma_2017_2000m <-mt_ma |>
  filter(Ano == "2017") |>
  st_buffer(dist = 2000)

bf_mt_ma_2016_2000m <-mt_ma |>
  filter(Ano == "2016") |>
  st_buffer(dist = 2000)

bf_mt_ma_2015_2000m <-mt_ma |>
  filter(Ano == "2015") |>
  st_buffer(dist = 2000)

bf_mt_ma_2014_2000m <-mt_ma |>
  filter(Ano == "2014") |>
  st_buffer(dist = 2000)

bf_mt_ma_2013_2000m <-mt_ma |>
  filter(Ano == "2013") |>
  st_buffer(dist = 2000)

bf_mt_ma_2012_2000m <-mt_ma |>
  filter(Ano == "2012") |>
  st_buffer(dist = 2000)

bf_mt_ma_2011_2000m <-mt_ma |>
  filter(Ano == "2011") |>
  st_buffer(dist = 2000)

bf_mt_ma_2010_2000m <-mt_ma |>
  filter(Ano == "2010") |>
  st_buffer(dist = 2000)
...
output_dir <- "ras_mt_ma_2016_2000m"
ras_mt_ma_2016_2000m <- list.files(output_dir, pattern = "\\.tif$", full.names = TRUE) %>%
  set_names(~ str_remove(basename(.), "ras_mt_ma_2016_2000m\\.tif$")) %>%
  map(rast) 
...
write_xlsx(lsm_mt_ma_2000m, "lsm_mt_ma_2000m.xlsx")

#lsm_mt_ma_2000m <- read_excel("D:/Analise estatistica mestrado/lsm_mt_ma_2000m.xlsx", sheet = 1,na = c(" ", "NA")) 

#### Distancia massa dagua ####

dist_agua_mt_ma <- st_distance(mt_ma, agua_ma)

mt_ma$dist_min_agua <- as.numeric(apply(dist_agua_mt_ma, 1, min))

dist_min_agua_ma_tt <- st_drop_geometry(mt_ma[, c("id_unico", "dist_min_agua")])
...
distancias_result <- calcular_dist_minima(mt_ma, UC)
...
den_500m <- data.frame(id_unico = lsm_mt_ma_500m$id_unico_500m) |>
  left_join(den_rod_500m, by = "id_unico") |>
  mutate(den_rod_500m = ifelse(is.na(den_rod_500m), 0, as.numeric(den_rod_500m)))

lsm_mt_ma_500m$den_rod_500m <- den_500m$den_rod_500m

den_rod <- st_drop_geometry(lsm_mt_ma_500m[, c("id_unico_500m", "den_rod_500m")])
...
den_rod_3500m <- buffer_3500m |> 
  ...
  mutate(comprimento_rod_3500m = ifelse(is.na(comprimento_rod_3500m), 0, comprimento_rod_3500m),
         den_rod_3500m = (comprimento_rod_3500m/1000) / ((3.14*3500^2)/1e6))
...
den_rod <- st_drop_geometry(lsm_mt_ma_3500m[, c("id_unico_3500m", "den_rod_3500m")])
...
den_rod <- st_drop_geometry(lsm_mt_ma_2000m[, c("id_unico_2000m", "den_rod_2000m")])

#### Unificando todas as tabelas ####

lsm_mt_ma_500m_uni <- lsm_mt_ma_500m |>
...
glimpse(c(lsm_mt_ma_500m_uni, lsm_mt_ma_3500m_uni, lsm_mt_ma_2000m_uni))
...
lsm_mt_ma_total <- lsm_mt_ma_500m_uni %>%
  ...
  left_join(dist_min_uc_mt_ma, by = 'id_unico')


colum_na <- lsm_mt_ma_total|>
...
id_cols <- lsm_mt_ma_total |> select(id_unico, Ano, Bin, median_shape_500m, median_shape_2000m)

num_cols <- lsm_mt_ma_total |> select(-id_unico, -Ano, -Bin, -median_shape_500m, -median_shape_2000m)
...
lsm_mt_ma_std <- bind_cols(id_cols, stan)

# Verificar o resultado
glimpse(lsm_mt_ma_std)

write_xlsx(lsm_mt_ma_std, "lsm_mt_ma_std.xlsx")

lsm_mt_ma_std$Bin <- as.factor(lsm_mt_ma_std$Bin)


var_mt_ma <- lsm_mt_ma_std 
...
num_var_mt_ma <- var_mt_ma|>
  dplyr::select(-id_unico, - Bin, -Ano, -median_shape_500m, -median_shape_2000m, -pland_12_500m, -pland_12_3500m, -pland_12_2000m)|>
  st_drop_geometry()


# 1. Obter os nomes das colunas
vars <- colnames(num_var_mt_ma)
...
# Extrair combinações
comb <- var_glm(combinacoes_mt_ma)
...
print(formulas_glm$ind_ed_3_3500m) 
...
modelos_vif<-preparar_mumin(modelos) # 355 → 332 modelos (VIF ≤ 5 )
...
roc_obj_1 <- roc(var_mt_ma_total$Bin, probs_1)
auc(roc_obj_1) # 0.5791

rcompanion::nagelkerke(melhor_modelo_1) # nagelkerke: 0.0489564 p-value: 5.9557e-12
...
roc_obj_2 <- roc(var_mt_ma_total$Bin, probs_2)
auc(roc_obj_2) # 0.583

rcompanion::nagelkerke(melhor_modelo_2) # nagelkerke: 0.0485961 p-value: 7.3009e-12

# Melhor modelo 3

melhor_modelo_3 <- MuMIn::get.models(aic_tabela, subset = 3)[[1]]
summary(melhor_modelo_3)
simulationOutput <- DHARMa::simulateResiduals(fittedModel = melhor_modelo_3, plot = TRUE) # OK

probs_3 <- predict(melhor_modelo_3, type = "response")
roc_obj_3 <- roc(var_mt_ma_total$Bin, probs_3)
auc(roc_obj_3) # 0.5775

rcompanion::nagelkerke(melhor_modelo_3) # nagelkerke: 0.0467352 p-value: 3.5366e-12

# Melhor modelo 4

melhor_modelo_4 <- MuMIn::get.models(aic_tabela, subset = 4)[[1]]
summary(melhor_modelo_4)
simulationOutput <- DHARMa::simulateResiduals(fittedModel = melhor_modelo_4, plot = TRUE) # OK

probs_4 <- predict(melhor_modelo_4, type = "response")
roc_obj_4 <- roc(var_mt_ma_total$Bin, probs_4)
auc(roc_obj_4) # 0.5763

rcompanion::nagelkerke(melhor_modelo_4) # nagelkerke: 0.0459150 p-value: 5.6641e-12
...
write_xlsx(tabela_final, "tabela_modelos_4.xlsx")
